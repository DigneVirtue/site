...
document.addEventListener("DOMContentLoaded", function () {
  // Função para verificar se a barra está visível na tela
  ...
  function animateBars() {
    ...
    progressBars.forEach(bar => {
      if (isInViewport(bar) && !bar.classList.contains('animated')) {
        ...
        const targetWidth = bar.getAttribute('data-progress'); // Porcentagem alvo da barra
        const label = bar.querySelector('.percentage'); // O elemento de porcentagem
        let currentWidth = 0;

        // Função para animar o aumento da barra
        const interval = setInterval(() => {
          if (currentWidth < parseInt(targetWidth)) {
            currentWidth++;
            bar.style.width = currentWidth + '%'; // Atualiza a largura da barra
            label.textContent = currentWidth + '%'; // Atualiza a porcentagem
          } else {
            clearInterval(interval); // Para a animação quando atingir o alvo
          }
        }, 20); // Ajuste de tempo para a animação
      }
    });
  }
  ...
  // Executa a animação quando o usuário rolar a página
